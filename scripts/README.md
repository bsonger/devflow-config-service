...
Regenerates Swagger and builds the Linux amd64 service binary at `bin/devflow-config-service`.
...
This script pins the `swag` tool through `go run` so the output stays reproducible.
