...
- `POST /api/v1/app-configs/{id}/sync-from-repo` reads all service runtime config files from repo, preserving repo file names, and applies optional `environments/{env}/*` file overlays into the final file set
